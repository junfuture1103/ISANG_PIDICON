...
def make_address_hierarchy():
    ...
    Address_Hierarchy = pd.concat([level3, level2, level1], axis = 1)
    
    print(Address_Hierarchy)
    
    read_data['주소'] = BIG_MIDDLE_address
    
    read_data.to_csv('ISANG_DATASETS.csv', header = None, encoding = 'ANSI', index = False)
